function varargout = funct_plot_google_map(varargin)
% function h = funct_plot_google_map(varargin)
% Plots a google map on the current axes using the Google Static Maps API
%
% USAGE:
% h = funct_plot_google_map(Property, Value,...)
% Plots the map on the given axes. Used also if no output is specified
%
% Or:
% [lonVec latVec imag] = funct_plot_google_map(Property, Value,...)
...
%                     To set the key, use:
%                     funct_plot_google_map('APIKey','SomeLongStringObtaindFromGoogle')
%                     You need to do this only once to set the key.
%                     To disable the use of a key, use:
%                     funct_plot_google_map('APIKey','')
%
% OUTPUT:
    ...
    for idx = 1:2:length(varargin)
        switch lower(varargin{idx})
            ...
            case 'apikey'
                ...
                % save key to file
                funcFile = which('funct_plot_google_map.m');
